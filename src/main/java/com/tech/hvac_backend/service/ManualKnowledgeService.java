...
public class ManualKnowledgeService {
    ...
    public List<ManualKnowledgeChunkEntity> findRelevantChunks(CfrDraftEntity draft) {
        return findRelevantChunks(new ManualSearchInput(
                ...
                draft.getMachineRefrigerant(),
                draft.getMachineOilType(),
                draft.getMachineControlSystem(),
    ...
    private String buildSearchQuery(ManualSearchInput input) {
        return String.join(" ",
                ...
                clean(input.machineRefrigerant()),
                clean(input.machineOilType()),
                clean(input.machineControlSystem()),
                clean(input.machineSoftwareVersion()),
                clean(input.machineCompressorType()),
                clean(input.machineMfg()),
    ...
    private List<String> detectTopics(ManualSearchInput input) {
        ...
        addIf(text, topics, "refrigerant_system",
                "refrigerant", "r134a", "r513a", "r407c", "charge", "leak");

        addIf(text, topics, "control_system",
                "control system", "software", "version", "controller", "plc", "panel", "hmi");

...
@Service
public class HealthCheckPromptBuilderService {

    private final ManualKnowledgeService manualKnowledgeService;

    public HealthCheckPromptBuilderService(ManualKnowledgeService manualKnowledgeService) {
        this.manualKnowledgeService = manualKnowledgeService;
    }

    public String buildPrompt(
            HealthCheckReportEntity report,
            MachineEntity machine,
            VesselEntity vessel,
            List<HealthCheckReportTaskEntity> tasks,
        ...
        return """
                You are generating a customer-ready Johnson Controls Marine & Navy Health Check Report.

                You are acting as an experienced HVAC/refrigeration field service engineer reviewing a completed health check checklist, technician notes, measured values, machine information, photos, and OEM manual references.

                Use the technician health check tasks as the primary field evidence.
                ...
                Do not copy manual text directly; synthesize it into clear health check explanations.
                ...
                Field rules:
                - title must be "Health Check Report".
                ...
                - machineStatus must be exactly one of these two strings: "Work Finished" or "Work In Progress".
                ...
                - healthCheckResult should state the overall result of the health check based on task statuses, faultCount, skippedCount, measured values, and notes.
                - alarmStatus should be "Alarm reported" when at least one task has status attention or fault, otherwise "No alarm reported".
                - executiveSummary should summarize health check scope + completed observations + abnormal findings + required action.
                - healthCheckSummary should be a concise customer-ready narrative of the health check activities and machine condition.
                ...
                - recommendations must be clear, actionable, and relevant to the health check result. Use manual context only when it directly supports the reported task or abnormal condition.
                - furtherActionRequired should summarize required follow-up, monitoring, return visit, parts, pending confirmation, or operational limitation based only on source data. If no follow-up is required, state that no further action is required based on the completed health check tasks.
                ...
                taskList.isBlank() ? "No health check tasks provided." : taskList,
                alarmTaskList.isBlank() ? "No attention or fault tasks." : alarmTaskList,
                photoList
        );
    }

    private String formatTask(HealthCheckReportTaskEntity task) {
        return """
                - Task Id: %s
                  Category: %s
                  Task: %s
    ...
    private boolean isAlarmTask(HealthCheckReportTaskEntity task) {
